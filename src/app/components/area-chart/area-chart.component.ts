...
export class AreaChartComponent {
  ...
  @ViewChild("clipRect", { static: true })
  clipRect!: ElementRef<SVGRectElement>;
  ...
  constructor(
    ...
    effect(() => {
      this.chartService
        .getAreaChartData(this.selectedRange())
        .subscribe((data) => {
  ...
  updateRange(event: MatSelectChange) {
    ...
    this.shouldAnimatePath.set(false);
  ...
  svgPath = computed(() => {
    ...
    const maxY = Math.max(...visibleData.flatMap((d) => [d.all, d.loyalty]));
    const width = 600;
    const height = 300;
  ...
  totalAll = computed(() => this.areaData().reduce((sum, d) => sum + d.all, 0));
